  ...
  return (
    <div className="flex flex-wrap gap-2">
      {roles.map((role) => (
        <span
          key={role}
          className={`inline-flex items-center rounded-full border px-2 py-0.5 text-xs font-medium ${ROLE_STYLES[role] ?? "border-stone-300 bg-stone-200/40 text-stone-900"}`}
        >
          {titleCase(role)}
        </span>
      ))}
